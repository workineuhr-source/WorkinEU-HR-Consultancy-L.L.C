...
import { Upload, X, FileText, CheckCircle2, User, ArrowRight, Sparkles, Plus, Trash2, Award } from 'lucide-react';
...
const PROFICIENCY_LEVELS = ["Beginner", "Intermediate", "Expert", "Fluent", "Native"];

const COMMON_SKILLS = [
  "English", "Driving", "Forklift Operation", "Customer Service", "First Aid", 
  "Microsoft Office", "Welding", "Security", "Cooking", "Cleaning", 
  "Inventory Management", "Quality Control"
];

export default function ApplicationForm({ job, onSuccess, autoFillIntent }: ApplicationFormProps) {
  ...
  const nextStep = async () => {
    ...
    if (step === 1) {
      fieldsToValidate = ['fullName', 'email', 'phone', 'passportNumber', 'nationality'];
    } else if (step === 2) {
      fieldsToValidate = ['experience', 'education', 'appliedPosition', 'appliedCountry'];
    }
    ...
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      if (user) {
        try {
          ...
          if (docSnap.exists()) {
            const profile = { uid: docSnap.id, ...docSnap.data() } as CandidateProfile;
  ...
  const applyProfileData = async () => {
    ...
    if (candidateProfile.fullName) setValue('fullName', candidateProfile.fullName);
    ...
    if (candidateProfile.passportNumber) setValue('passportNumber', candidateProfile.passportNumber);
    if (candidateProfile.nationality) setValue('nationality', candidateProfile.nationality);
    if (candidateProfile.experience) setValue('experience', candidateProfile.experience);
    if (candidateProfile.education) setValue('education', candidateProfile.education);
    ...
    await trigger(['fullName', 'email', 'phone', 'passportNumber', 'nationality', 'experience', 'education']);
  ...
  if (submitted) {
    return (
      <div className="text-center py-20">
        ...
        <h2 className="text-5xl font-bold text-slate-900 dark:text-white mb-6 tracking-tighter">Application Received!</h2>
        <p className="text-slate-500 dark:text-slate-400 text-xl font-light mb-12 max-w-lg mx-auto leading-relaxed">
          Thank you for applying for the <span className="text-brand-gold font-bold">{job.title}</span> position. Our HR team will review your profile and get back to you soon.
        </p>
          ...
          {steps.map((s) => (
            <div key={s.id} className="flex flex-col items-center gap-3 relative z-10">
              <div className={cn(
                ...
                step >= s.id ? "bg-slate-900 dark:bg-brand-gold text-brand-gold dark:text-brand-blue scale-110" : "bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500"
              )}>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              {candidateProfile && (
                ...
                >
                  ...
                  <div className="flex items-center gap-6 relative z-10">
                    ...
                    <div>
                      <p className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Apply Faster with Your Profile</p>
                      <p className="text-base text-slate-600 dark:text-slate-400 font-medium">We found your saved information. Click to pre-fill the form instantly.</p>
                    </div>
                  </div>
                  <motion.button 
                  ...
                  >
                    <User size={24} className="text-brand-gold dark:text-brand-blue group-hover/btn:text-slate-900 transition-colors" />
                    Apply with Profile
                  </motion.button>
                </motion.div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Full Name</label>
                  <input 
                    {...register('fullName')}
                  ...
                  />
                  {errors.fullName && <p className="text-red-500 text-xs mt-2 font-bold ml-6 uppercase tracking-tighter">{errors.fullName.message}</p>}
                </div>
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Email Address</label>
                  <input 
                    {...register('email')}
                    className="w-full px-10 py-6 rounded-[2rem] border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 outline-none focus:bg-white dark:focus:bg-slate-800 focus:border-brand-gold focus:ring-8 focus:ring-brand-gold/5 transition-all duration-500 text-slate-900 dark:text-white font-bold shadow-inner text-lg"
                    placeholder="john@example.com"
                  />
                  {errors.email && <p className="text-red-500 text-xs mt-2 font-bold ml-6 uppercase tracking-tighter">{errors.email.message}</p>}
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Phone Number</label>
                  <input 
                    {...register('phone')}
                    className="w-full px-10 py-6 rounded-[2rem] border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 outline-none focus:bg-white dark:focus:bg-slate-800 focus:border-brand-gold focus:ring-8 focus:ring-brand-gold/5 transition-all duration-500 text-slate-900 dark:text-white font-bold shadow-inner text-lg"
                    placeholder="+971 ..."
                  />
                  {errors.phone && <p className="text-red-500 text-xs mt-2 font-bold ml-6 uppercase tracking-tighter">{errors.phone.message}</p>}
                </div>
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Passport Number</label>
                  <input 
                    {...register('passportNumber')}
                    className="w-full px-10 py-6 rounded-[2rem] border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 outline-none focus:bg-white dark:focus:bg-slate-800 focus:border-brand-gold focus:ring-8 focus:ring-brand-gold/5 transition-all duration-500 text-slate-900 dark:text-white font-bold shadow-inner text-lg"
                    placeholder="Enter passport number"
                  />
                  {errors.passportNumber && <p className="text-red-500 text-xs mt-2 font-bold ml-6 uppercase tracking-tighter">{errors.passportNumber.message}</p>}
                </div>
              </div>

              <div className="space-y-3">
                <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Nationality</label>
                <input 
                ...
                {errors.nationality && <p className="text-red-500 text-xs mt-2 font-bold ml-6 uppercase tracking-tighter">{errors.nationality.message}</p>}
              </div>
            </motion.div>
          )}
          ...
          {step === 2 && (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Years of Experience</label>
                  <select 
                    {...register('experience')}
                    className="w-full px-10 py-6 rounded-[2rem] border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 outline-none focus:bg-white dark:focus:bg-slate-800 focus:border-brand-gold focus:ring-8 focus:ring-brand-gold/5 transition-all duration-500 text-slate-900 dark:text-white font-bold shadow-inner appearance-none cursor-pointer text-lg"
                  >
                    <option value="" className="dark:bg-slate-800">Select Experience</option>
                    <option value="Entry Level" className="dark:bg-slate-800">Entry Level</option>
                    <option value="1-3 Years" className="dark:bg-slate-800">1-3 Years</option>
                    <option value="3-5 Years" className="dark:bg-slate-800">3-5 Years</option>
                    <option value="5-10 Years" className="dark:bg-slate-800">5-10 Years</option>
                    <option value="10+ Years" className="dark:bg-slate-800">10+ Years</option>
                  </select>
                  {errors.experience && <p className="text-red-500 text-xs mt-2 font-bold ml-6 uppercase tracking-tighter">{errors.experience.message}</p>}
                </div>
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Highest Education</label>
                  <input 
                    {...register('education')}
                  ...
                  {errors.education && <p className="text-red-500 text-xs mt-2 font-bold ml-6 uppercase tracking-tighter">{errors.education.message}</p>}
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Position Applied For</label>
                  <input 
                  ...
                  {errors.appliedPosition && <p className="text-red-500 text-xs mt-2 font-bold ml-6 uppercase tracking-tighter">{errors.appliedPosition.message}</p>}
                </div>
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Country Applied For</label>
                  <input 
                  ...
                  {errors.appliedCountry && <p className="text-red-500 text-xs mt-2 font-bold ml-6 uppercase tracking-tighter">{errors.appliedCountry.message}</p>}
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3 ml-6 mb-2">
                  ...
                  <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Key Skills & Proficiency</label>
                </div>
                ...
                <div className="bg-slate-50 dark:bg-white/5 p-8 rounded-[3rem] border border-slate-100 dark:border-white/10 space-y-6">
                  <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    >
                      {PROFICIENCY_LEVELS.map(l => <option key={l} value={l} className="dark:bg-slate-800">{l}</option>)}
                    </select>
                    <button 
                      type="button"
                      onClick={addSkill}
                    ...
                    <div className="flex flex-wrap gap-3 pt-4">
                      {skills.map((skill, index) => (
                        ...
                        >
                          ...
                          <span className="text-[10px] font-black uppercase text-brand-gold bg-brand-gold/10 px-3 py-1 rounded-full">{skill.level}</span>
                          <button 
                            type="button"
          ...
          {step === 3 && (
            ...
            >
              <div className="space-y-3">
                <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Cover Letter (Optional)</label>
                <textarea 
              ...
              <div className="space-y-6">
                <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-6">Upload Documents (Passport, CV, Certificates)</label>
                  ...
                  {files.map((file, i) => (
                    ...
                    >
                      <div className="flex items-center gap-4 overflow-hidden">
                        ...
                        <span className="text-sm font-bold text-slate-900 dark:text-white truncate">{file.name}</span>
                      </div>
                      <button 
                        type="button"
                ...
                <label className="flex flex-col items-center justify-center w-full h-56 border-4 border-dashed border-slate-100 dark:border-white/5 rounded-[4rem] cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5 hover:border-brand-gold transition-all duration-700 group relative overflow-hidden">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6 relative z-10">
                    ...
                    <p className="text-sm text-slate-400 dark:text-slate-500 font-bold uppercase tracking-[0.3em] mb-2">Upload Files</p>
                    <p className="text-xs text-slate-300 dark:text-slate-500 font-medium">PDF, PNG, JPG (Max 5MB each)</p>
                  </div>
