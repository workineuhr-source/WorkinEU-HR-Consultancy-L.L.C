...
export default function BottomCTA() {
  ...
  return (
    <section className="py-12 relative overflow-hidden bg-white dark:bg-[#121212] mt-12 border-t border-slate-100">
        ...
        <div className="w-full bg-slate-50 dark:bg-slate-800/50 p-6 md:p-10 border border-slate-100 dark:border-white/5 shadow-sm">
          ...
          >
            <div className="flex-1 space-y-4 text-center md:text-left">
              ...
              <h2
                className={`text-2xl md:text-3xl lg:text-4xl font-black leading-[1.1] tracking-tight ${getFontClass(content.styles?.bottomCta?.font)} ${getTextColor(content.styles?.bottomCta?.titleColor)}`}
              >
                {content.ctaTitle || "Ready to Start Your Global Career?"}
              </h2>

              <p
                className={`text-sm md:text-base leading-relaxed max-w-xl mx-auto md:mx-0 font-medium ${getFontClass(content.styles?.bottomCta?.font)} ${getTextColor(content.styles?.bottomCta?.descriptionColor, "text-slate-500")}`}
              >
                {content.ctaDescription ||
                  "Join hundreds of successful candidates who have transformed their lives through WorkinEU's expert recruitment."}
              </p>
            </div>
            ...
            <div className="shrink-0 flex items-center justify-center">
              ...
              >
                {content.ctaButtonText || "Explore All Jobs"}{" "}
