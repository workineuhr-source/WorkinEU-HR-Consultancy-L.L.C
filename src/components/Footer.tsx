...
export default function Footer() {
    ...
    const unsub = onSnapshot(doc(db, 'settings', 'siteContent'), (snap) => {
      if (snap.exists()) {
        ...
        setContactInfo({
          ...
          address: data.contactAddress || 'Mai Tower 4th Floor, Office No. 10, Al Qusais, Al Nahda 1, Dubai, UAE',
  ...
  return (
    <footer className="bg-[#020617] text-white pt-24 pb-12 relative overflow-hidden transition-colors duration-500">
      ...
      <div className="max-w-[1920px] mx-auto px-4 md:px-8 lg:px-12 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-16 mb-20">
          ...
          <div className="lg:col-span-4 space-y-10">
            <Link to="/" className="inline-block group">
              <div className="flex items-center gap-5">
                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-2xl group-hover:rotate-6 transition-transform overflow-hidden">
                  <img src={footerLogoUrl || logoUrl || "/logo.png"} alt="WorkinEU HR" className="w-full h-full object-contain p-1" referrerPolicy="no-referrer" />
                </div>
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.4em] text-brand-teal font-black mt-2">Consultancy LLC</span>
                </div>
              </div>
            </Link>
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-5">
              {[
                ...
                <li key={link.name}>
                  <Link to={link.path} className="text-slate-300 hover:text-white transition-all flex items-center gap-4 group text-xs font-black uppercase tracking-widest">
          ...
          <div className="lg:col-span-4">
            ...
            <div className="grid grid-cols-1 gap-6">
              {(contactInfo.branchOffices && contactInfo.branchOffices.length > 0 ? contactInfo.branchOffices : [
                { location: 'Main Office', address: contactInfo.address, phone: contactInfo.phone, email: contactInfo.email }
              ]).map((branch, i) => (
                <div key={i} className="bg-white/5 border border-white/5 p-8 rounded-3xl hover:bg-white/10 transition-all group border-l-4 border-l-brand-teal">
                  <h5 className="text-brand-teal font-black text-[10px] uppercase tracking-[0.3em] mb-6">{branch.location}</h5>
                  <div className="grid grid-cols-1 gap-4 text-slate-200 text-sm font-medium">
                    <div className="flex items-start gap-4">
                      <MapPin className="text-brand-teal shrink-0 mt-0.5" size={16} />
                      <span className="leading-relaxed opacity-70">{branch.address}</span>
                    </div>
        ...
        <div className="border-t border-white/10 pt-12 flex flex-col md:flex-row justify-between items-center gap-10 text-slate-400 text-[10px] font-black uppercase tracking-[0.3em]">
          ...
          <div className="flex items-center gap-10">
            ...
            <p className="hover:text-white transition-colors cursor-default">Designed by Udaya Raj Khanal</p>
          </div>
        </div>
      </div>
    </footer>
