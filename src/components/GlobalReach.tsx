...
export default function GlobalReach({ 
  ...
  const visibleCountries = expanded ? countries : countries.slice(0, INITIAL_COUNT);
      ...
      <div className="max-w-[1920px] mx-auto px-4 md:px-8 relative z-10">
        <div className="max-w-4xl mb-20 md:mb-32">
          ...
          <motion.h2 
            ...
            className={cn("text-4xl md:text-7xl lg:text-9xl font-black tracking-tighter mb-8 md:mb-12 font-sans", dark ? "text-white" : "text-slate-900")}
          >
            {title.split(' ').map((word, i) => (
              <span key={i} className={i === 1 ? "text-brand-teal italic" : ""}>{word} </span>
            ))}
          </motion.h2>
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            ...
            className={cn("text-lg md:text-2xl font-medium leading-relaxed max-w-3xl", dark ? "text-slate-400" : "text-slate-500")}
          >
            {subtitle}
          </motion.div>
        </div>
            ...
            {visibleCountries.map((country, i) => (
              ...
              >
                ...
                >
                  ...
                  {jobCounts[country] > 0 && (
                    <div className="absolute top-4 right-4 bg-white text-[#020617] text-[10px] font-black px-3 py-1 rounded-full z-20 shadow-lg transition-transform group-hover:scale-110">
                      {jobCounts[country]}
                    </div>
                  )}
                  ...
                  <p className={cn(
                    ...
                    dark ? "text-white group-hover:text-white" : "text-slate-900 group-hover:text-white"
                  )}>
                    {country}
                  </p>
                  <div className="flex items-center justify-center gap-2 relative z-10">
                    <div className={cn(
                      ...
                      jobCounts[country] > 0 ? "bg-brand-teal" : "bg-slate-500",
                      "group-hover:bg-white"
                    )}></div>
                    <p className={cn(
                      "text-[10px] font-black uppercase tracking-widest transition-colors duration-500",
                      dark ? "text-slate-400 group-hover:text-white" : "text-slate-400 group-hover:text-white"
                    )}>
                      {jobCounts[country] ? `${jobCounts[country]} Opportunities` : 'New Roles Soon'}
                    </p>
                  </div>
                </Link>
              </motion.div>
          ...
          >
            <motion.button
              ...
              className={cn(
                ...
                dark 
                  ? "bg-white text-[#020617] hover:bg-brand-teal hover:text-white" 
                  : "bg-[#020617] text-white hover:bg-brand-teal"
              )}
            >
              {expanded ? (
                <>
