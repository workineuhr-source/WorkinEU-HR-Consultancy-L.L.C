      ...
      if (snap.exists()) {
        ...
        if (active.length > 0) {
          ...
          const defaultRaj: AIAssistant = {
            ...
            systemPrompt:
              "You are RAJ, the Senior HR Consultant at WorkinEU HR Consultancy. Your tone MUST be professional, direct, knowledgeable, and polite—exactly like a senior HR professional.",
            isActive: true,
            color: "#1e293b",
          };
...
- Website: workineu.com`
        : "Standard WorkinEU guidance applies.";
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          text:
            response.text ||
            "I am currently adjusting my schedule. Please try again in a moment.",
          timestamp: Date.now(),
          assistantId: selectedAssistant.id,
        },
      ]);
      ...
      setMessages((prev) => [
        ...
        {
          role: "bot",
          text: `I'm having some trouble connecting right now. Please reach out to our team via WhatsApp for direct assistance.`,
          timestamp: Date.now(),
        },
      ]);
    } finally {
  ...
  const activeAssistants =
    siteContent?.assistants?.filter((a) => a.isActive) ||
    ([selectedAssistant].filter(Boolean) as AIAssistant[]);
  ...
  return (
    <>
      ...
      >
        ...
        <span className="absolute right-full mr-4 bg-white dark:bg-[#121212] text-brand-blue px-4 py-2 rounded-xl font-bold text-sm shadow-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none border border-gray-100">
          WorkinEU Assistant
        </span>
      </button>
        ...
        {isOpen && (
          <div className="fixed inset-0 z-[100] md:inset-auto md:bottom-28 md:right-8 md:w-[450px] md:h-[700px] flex flex-col p-0 md:p-0">
            ...
            >
              {/* Header */}
              <div
                className="bg-slate-900 p-8 flex justify-between items-center text-white relative overflow-hidden transition-colors duration-500"
                style={{
                  backgroundColor: selectedAssistant?.color || "#121212",
                }}
              >
                ...
                <div className="flex items-center gap-4 relative z-10">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-white/50 font-bold uppercase tracking-widest">
                      {selectedAssistant?.role || "Expert Advisor"}
                    </p>
                  </div>
                </div>
                <button
              ...
              {activeAssistants.length > 1 && (
                <div className="bg-slate-50 border-b border-slate-100 p-4 flex gap-3 overflow-x-auto no-scrollbar scroll-smooth">
                  {activeAssistants.map((asst) => (
                    ...
                    >
                      <div className="w-5 h-5 rounded-lg overflow-hidden border border-slate-100 bg-slate-50 dark:bg-white/5 flex items-center justify-center">
                        {asst.photoUrl ? (
                          <img
                            referrerPolicy="no-referrer"
                          ...
                          <div
                            ...
                            style={{ backgroundColor: asst.color }}
                          ></div>
                        )}
                      </div>
                      {asst.name}
              ...
              <div className="flex-grow overflow-y-auto p-8 space-y-6 bg-slate-50/50">
                {messages.map((msg, i) => {
                  const asst = activeAssistants.find(
                    (a) => a.id === msg.assistantId,
                  );
                  return (
                    <motion.div
                      ...
                      <div
                        className={cn(
                          ...
                          msg.role === "user"
                            ...
                            : "bg-white border-slate-100 text-slate-800 dark:text-slate-200",
                        )}
                      >
                        {msg.role === "user" ? (
                          <User size={18} />
                      ...
                      <div
                        className={cn(
                          ...
                          msg.role === "user"
                            ...
                            : "bg-white text-slate-700 dark:text-slate-300 border border-slate-100 rounded-tl-none",
                        )}
                      >
                        {msg.text}
                      </div>
                ...
                {isLoading && (
                  <div className="flex gap-4 max-w-[90%]">
                    <div className="w-10 h-10 bg-white dark:bg-[#121212] border border-slate-100 text-slate-400 rounded-xl flex items-center justify-center shrink-0 shadow-sm overflow-hidden">
                      {selectedAssistant?.photoUrl ? (
                        <img
                          referrerPolicy="no-referrer"
                          src={getDirectImageUrl(selectedAssistant.photoUrl)}
              ...
              <form
                onSubmit={handleSend}
                className="p-8 bg-white dark:bg-[#121212] border-t border-slate-100"
              >
                <div className="relative group">
                  <div
                    className="absolute inset-0 rounded-2xl blur-lg opacity-0 group-focus-within:opacity-100 transition-opacity"
                    style={{ backgroundColor: `${selectedAssistant?.color}10` }}
                  ></div>
                  <input
                    type="text"
                    value={message}
                    ...
                    className="w-full pl-6 pr-14 py-5 rounded-2xl border border-slate-200 outline-none focus:ring-4 transition-all bg-white dark:bg-[#121212] relative z-10 font-bold"
                    style={{
                      borderColor: selectedAssistant?.color
                        ? `${selectedAssistant.color}30`
                        : undefined,
                    }}
                  />
                  <button
                    type="submit"
                    disabled={
                      !message.trim() || isLoading || !selectedAssistant
                    }
