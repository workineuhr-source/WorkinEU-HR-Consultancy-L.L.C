...
export default function CandidateProfilePage() {
  ...
  const handleDownload = async (
    ...
    setTimeout(async () => {
      try {
        ...
        pdf.save(
          `Europass_CV_${profile.fullName.replace(/\s+/g, "_")}_${theme}.pdf`,
        );
      } catch (error) {
  ...
  if (!profile) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-white/5 p-4 font-sans">
        <div className="text-center bg-white dark:bg-[#121212] p-12 rounded-3xl shadow-xl max-w-md border border-gray-100">
          <User className="mx-auto text-gray-300 mb-6" size={64} />
          <h2 className="text-2xl font-bold text-brand-blue mb-4">
            Profile Not Found
          </h2>
          <Link to="/" className="text-brand-gold font-bold hover:underline">
            Back to Home
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-white/5 print:bg-white pb-20 pt-28 font-sans">
      <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                Select a theme to update your CV
              </p>
            </div>
          </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedTheme === themeOption.id
                    ...
                    : "bg-gray-50 text-gray-500 dark:text-gray-300 hover:bg-gray-100 hover:text-brand-blue",
                )}
              >
                {themeOption.name}
              </button>
        ...
        <div className="fixed -left-[10000px] top-0 pointer-events-none">
          <EuropassCV
            id="europass-cv-download-template"
            candidate={profile}
            theme={selectedTheme}
          />
        </div>
