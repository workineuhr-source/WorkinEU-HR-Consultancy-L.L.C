...
export default function JobDetailsPage() {
  ...
  const diffTime = job.deadline ? deadlineDate.getTime() - today.getTime() : 1;
      ...
      <div className="max-w-[1700px] mx-auto px-4 md:px-8 lg:px-12 relative z-10">
        ...
        >
          <div className="flex items-center gap-6 mb-12">
            <Link
              to="/jobs"
              className="inline-flex items-center gap-3 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all font-bold uppercase tracking-[0.3em] text-[10px] group"
            >
            ...
            <Link
              to="/"
              className="inline-flex items-center gap-3 text-slate-500 dark:text-slate-400 hover:text-brand-teal transition-all font-bold uppercase tracking-[0.3em] text-[10px] group"
            >
              ...
              <div className="absolute bottom-12 left-12 right-12">
                <div className="flex flex-wrap items-end justify-between gap-8">
                  ...
                  <div className="flex flex-wrap gap-4">
                    <div className="px-6 py-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl flex-grow md:flex-grow-0">
                      ...
                      <div className="px-6 py-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl flex-grow md:flex-grow-0">
                        ...
                        <p className="text-white font-black text-lg">
                          ...
                          {job.vacanciesMale || job.vacanciesFemale
                            ? <span className="text-sm font-medium text-white/90 ml-2 block sm:inline">({job.vacanciesMale ? `M: ${job.vacanciesMale}` : ''}{job.vacanciesMale && job.vacanciesFemale ? ', ' : ''}{job.vacanciesFemale ? `F: ${job.vacanciesFemale}` : ''})</span>
                            : null}
                        </p>
                      </div>
                    )}
              ...
              <div className="relative z-10 space-y-10 flex-grow">
                ...
                <div className="flex flex-col gap-4">
                  {isClosed ? (
                    <button
                      disabled
                      className="w-full bg-slate-100 dark:bg-slate-800 text-slate-400 py-6 rounded-3xl font-black text-xl flex items-center justify-center gap-3 cursor-not-allowed"
                    >
                      <X size={24} />
                      Job Closed
                    </button>
                ...
                <div className="mt-auto pt-10 border-t border-slate-100 dark:border-white/5 flex items-center justify-between">
                  ...
                  <div className="flex -space-x-3">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="w-8 h-8 rounded-full border-2 border-white dark:border-slate-800 bg-slate-100 dark:bg-slate-700 overflow-hidden ring-1 ring-slate-200 dark:ring-white/10"
                      >
                        <img
                          referrerPolicy="no-referrer"
          ...
          <div className="lg:col-span-8 space-y-10">
            ...
            >
              <div className="flex border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/50">
                {(
                  ["description", "responsibilities", "requirements"] as const
                ).map((tab) => (
                  <button
                    key={tab}
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 relative z-10">
                {job.requiredDocuments.map((doc, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-4 bg-white/5 border border-white/10 p-5 rounded-2xl hover:bg-white/10 transition-all group/doc"
                  >
                    <FileText
                      size={20}
                      className="text-brand-gold group-hover/doc:scale-110 transition-transform"
                    />
          ...
          <div className="lg:col-span-4 space-y-8 lg:sticky lg:top-32">
            ...
            >
              ...
              <div className="space-y-6 mb-10 relative z-10">
                <div className="flex items-center gap-6 p-5 bg-slate-50 dark:bg-white/5 rounded-3xl border border-slate-100 dark:border-white/10">
                  ...
                  <div>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-[0.2em] mb-1">
                      Final Deadline
                    </p>
            ...
            {(() => {
              ...
              const hasPricing =
                ...
                pricing.visaFeeMax;
              const hasItems =
                job.includedPackageItems?.length ||
                job.excludedPackageItems?.length;

              if (job.hideFeeDetails || (!hasPricing && !hasItems)) return null;
                ...
                >
                  <div className="flex flex-col gap-6 mb-8">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-[10px] font-bold text-brand-gold uppercase tracking-[0.4em]">
                          Fees & Package
                        </h4>
                      </div>
                    </div>
                  ...
                  <div className="space-y-6 mb-8">
                    {pricing.totalAmount ? (
                      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-brand-gold/20 shadow-sm relative overflow-hidden group">
                        ...
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2 relative z-10">
                          Total Amount
                        </span>
                        <span className="text-3xl font-black text-brand-blue dark:text-brand-gold tracking-tighter relative z-10">
                          {pricing.currency || job.currency || "€"}{" "}
                          {pricing.totalAmount}
                        </span>
                      </div>
                    ) : (
                      <div className="text-center py-8 px-4 bg-slate-50/50 dark:bg-white/5 rounded-2xl border border-dashed border-slate-200 dark:border-white/10">
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                          Fees not specified for {selectedRegion}
                        </p>
                      </div>
                    )}
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {pricing.initialPay && (
                          <div className="flex justify-between items-center p-3 px-4 bg-white/40 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-white/5">
                            <span className="text-slate-600 dark:text-slate-400 font-bold text-[10px] uppercase tracking-widest">
                              Initial Pay
                            </span>
                    ...
                    {(pricing.visaFeeMin || pricing.visaFeeMax) && (
                      <div className="p-5 bg-yellow-500/5 dark:bg-yellow-500/10 rounded-2xl border border-yellow-500/10 mt-6">
                        ...
                        <p className="text-[10px] text-slate-500 font-medium italic leading-relaxed">
                          Estimated external costs for {selectedRegion}{" "}
                          processing.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="p-5 bg-rose-500/5 dark:bg-rose-500/10 rounded-2xl border border-rose-500/10 mt-6 group">
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-400 font-medium leading-relaxed italic">
                        Note: In case of visa refusal, the risk amount is{" "}
                        <span className="font-bold text-rose-600">
                          {pricing.currency || job.currency || "€"}{" "}
                          {pricing.riskAmount || "500"}
                        </span>
                        . However, we offer full support to reapply again.{" "}
                        <strong>Reapplication for the visa is possible.</strong>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-8">
                    {job.includedPackageItems &&
                      job.includedPackageItems.filter((i) => i.trim()).length >
                        0 && (
                          ...
                          <ul className="space-y-2">
                            {job.includedPackageItems
                              ...
                              .map((item, i) => (
                                <li
                                  key={i}
                                  className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-3"
                                >
                    ...
                    {job.excludedPackageItems &&
                      job.excludedPackageItems.filter((i) => i.trim()).length >
                        0 && (
                        <div className="space-y-4">
                  ...
                  {pricing.riskAmount && (
                    <div className="mt-10 pt-8 border-t border-brand-gold/10 relative px-2">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[10px] font-black text-orange-600 uppercase tracking-[0.25em]">
                          Visa Refusal Policy
                        </span>
                      </div>
                      <div className="bg-orange-500/5 p-5 rounded-2xl border border-orange-500/10">
                        ...
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium italic">
                          However, we do reapply again. Reapplication for the
                          visa is possible.
                        </p>
                      </div>
                    </div>
