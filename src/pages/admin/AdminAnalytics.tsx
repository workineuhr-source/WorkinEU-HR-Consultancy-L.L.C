...
export default function AdminAnalytics() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setJobs(
        jobsSnap.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as Job),
      );
      setCandidates(
        candidatesSnap.docs.map(
  ...
  const getMonthlyTrendData = () => {
    ...
    return last6Months.map((month) => {
      const count = applications.filter((app) =>
        isSameMonth(new Date(app.createdAt), month),
      ).length;
      return {
  ...
  if (loading) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-gray-500 dark:text-gray-300 font-medium animate-pulse">
          Loading in-depth insights...
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-300">
            Comprehensive data visualization of your recruitment platform.
          </p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={fetchData}
            className="p-3 bg-white dark:bg-[#121212] border border-gray-100 rounded-xl text-gray-500 dark:text-gray-300 hover:text-brand-blue hover:border-brand-blue/20 transition-all shadow-sm"
            title="Refresh Data"
          >
            <RefreshCw size={20} />
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          {
            label: "Success Rate",
            value: `${Math.round((applications.filter((a) => a.status === "approved").length / (applications.length || 1)) * 100)}%`,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Application Trend */}
        <div className="lg:col-span-2 bg-white dark:bg-[#121212] p-8 rounded-3xl shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-8">
            ...
            <select className="text-xs font-bold text-gray-400 bg-gray-50 dark:bg-white/5 border-none rounded-lg px-3 py-1.5 outline-none">
            ...
            <div className="space-y-6">
              <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                ...
                <p className="text-xs text-white/60 mt-1">
                  Leading country in total applications received.
                </p>
              </div>
              <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                <p className="text-xs font-bold text-brand-gold uppercase tracking-widest mb-1">
                ...
                <p className="text-lg font-bold">
                  {getJobCategoryData().sort((a, b) => b.value - a.value)[0]
                    ?.name || "N/A"}
                </p>
                <p className="text-xs text-white/60 mt-1">
                  Most popular job category among applicants.
                </p>
              </div>
              <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                <p className="text-xs font-bold text-brand-gold uppercase tracking-widest mb-1">
                ...
                <p className="text-xs text-white/60 mt-1">
                  Application volume has grown by 15% over the last quarter.
                </p>
              </div>
            </div>
          </div>
