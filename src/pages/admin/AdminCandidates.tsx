...
export default function AdminCandidates() {
  ...
  const [assignModalCandidate, setAssignModalCandidate] =
  ...
  const uniqueCountries = Array.from(
    new Set(candidates.map((c) => c.assignedCountry).filter(Boolean)),
  ) as string[];
  const uniqueBatches = Array.from(
    new Set(candidates.map((c) => c.assignedBatch).filter(Boolean)),
  ) as string[];
  const uniqueCompanies = Array.from(
    new Set(candidates.map((c) => c.assignedCompany).filter(Boolean)),
  ) as string[];
  ...
  const fetchCandidates = async () => {
    try {
      ...
      const data = snapshot.docs.map(
        (doc) => ({ uid: doc.id, ...doc.data() }) as CandidateProfile,
      );
      setCandidates(data);
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          pdf.save(
            `Europass_CV_${(candidate.fullName || "Candidate").replace(/\s+/g, "_")}.pdf`,
          );
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-300 text-xs md:text-base">
            Manage users who have registered through the candidate portal.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-3 md:p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4">
        ...
        <div className="relative min-w-[200px]">
          ...
          />
          <select
            className="w-full pl-12 pr-4 py-2 md:py-3 rounded-xl border border-gray-100 focus:border-brand-gold outline-none transition-all text-sm md:text-base appearance-none bg-white dark:bg-[#121212] font-semibold text-gray-700 dark:text-gray-200"
            value={filterCountry}
        ...
        {loading ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100 animate-pulse h-64"
              ></div>
            ))}
          </div>
        ) : (
              ...
              <div key={country} className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
                  {groupCandidates.map((candidate) => (
                    <div
                      key={candidate.uid}
                      className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-all group flex flex-col"
                    >
                      <div className="flex items-start justify-between gap-4 mb-6">
                        <div className="flex items-start gap-4 flex-grow">
                          ...
                          <div className="flex-1 min-w-0 pt-1">
                            <h3 className="font-bold text-brand-blue text-sm md:text-lg break-words leading-tight">
                              {candidate.fullName || "Candidate Name"}
                            </h3>
                            <p className="text-[10px] md:text-xs text-gray-400 break-all mt-1">
                              {candidate.email}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="space-y-3 mb-6 flex-grow flex flex-col">
                        <div className="flex items-start gap-3 text-xs md:text-sm text-gray-500 dark:text-gray-300">
                          <Phone
                            size={14}
                            className="text-brand-gold shrink-0 mt-0.5 md:w-4 md:h-4"
                          />
                        ...
                        {(candidate.initialPay ||
                          candidate.payAfterWP ||
                          candidate.payAfterVisa) && (
                          <div className="pt-3 grid grid-cols-3 gap-2 border-t border-slate-50 mt-3">
                            <div className="text-center bg-slate-50 dark:bg-white/5 rounded-lg p-2 flex flex-col justify-center">
                              <p className="text-[8px] font-bold text-gray-400 uppercase leading-tight mb-1">
                                Initial
                              </p>
                              <p className="text-[10px] md:text-xs font-black text-brand-blue break-all">
                                {
                                  CURRENCY_SYMBOLS[
                                    candidate.paymentCurrency || "EUR"
                                  ]
                                }
                                {candidate.initialPay || "0"}
                              </p>
                            </div>
                            <div className="text-center bg-slate-50 dark:bg-white/5 rounded-lg p-2 flex flex-col justify-center">
                              <p className="text-[8px] font-bold text-gray-400 uppercase leading-tight mb-1">
                          ...
                          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <div className="flex justify-between items-center mb-3">
                              ...
                              <button
                                ...
                                className="text-brand-blue hover:text-brand-gold text-xs transition-colors p-1 bg-white dark:bg-[#121212] rounded-md shadow-sm border border-slate-100"
                            ...
                            {candidate.assignedCountry ? (
                              <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-100 relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-2 text-sm font-bold text-slate-800 dark:text-slate-200 break-words mb-2 pl-1">
                                  {candidate.assignedCountry}
                                </div>
                                {(candidate.assignedCompany ||
                                  candidate.assignedBatch) && (
                                  <div className="flex flex-col gap-2 pl-1">
                                    {candidate.assignedCompany && (
                                      <span className="flex items-start gap-2 text-[11px] text-gray-500 dark:text-gray-300 font-bold uppercase leading-tight">
                                        <Building2
                                          size={12}
                              ...
                              <div className="text-[10px] text-gray-400 font-bold uppercase tracking-widest text-center py-3 border border-dashed border-gray-200 rounded-xl bg-white dark:bg-[#121212]">
                                Not Assigned
                              </div>
                            )}
                          </div>
              ...
              <div className="flex justify-between items-start mb-6 border-b border-gray-100 pb-4">
                <div>
                  ...
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                    {assignModalCandidate.fullName}
                  </p>
                </div>
                <button
                  onClick={() => setAssignModalCandidate(null)}
                  className="p-2 bg-gray-50 dark:bg-white/5 text-gray-400 hover:text-brand-blue hover:bg-gray-100 rounded-full transition-colors"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <form onSubmit={handleSaveAssignment} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAssignData({ ...assignData, country: e.target.value })
                    }
                  />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAssignData({ ...assignData, batch: e.target.value })
                      }
                    />
                ...
                <div className="pt-2 flex gap-4 mt-8">
                  <button
                    ...
                    className="flex-grow px-6 py-3.5 rounded-xl font-bold text-gray-500 dark:text-gray-300 hover:bg-gray-50 transition-all border border-gray-200 uppercase tracking-widest text-xs"
                  >
                    Cancel
                  </button>
                  <button
