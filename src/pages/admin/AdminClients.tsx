...
export default function AdminClients() {
  ...
  useEffect(() => {
    const q = query(collection(db, 'clients'), orderBy('order', 'asc'), orderBy('createdAt', 'desc'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setClients(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ClientPartner)));
      setLoading(false);
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleMove = async (client: ClientPartner, direction: 'up' | 'down') => {
    const currentIndex = clients.findIndex(c => c.id === client.id);
    if (direction === 'up' && currentIndex > 0) {
      const prevClient = clients[currentIndex - 1];
      await setDoc(doc(db, 'clients', client.id), { ...client, order: prevClient.order }, { merge: true });
      await setDoc(doc(db, 'clients', prevClient.id), { ...prevClient, order: client.order }, { merge: true });
      toast.success('Order updated');
    } else if (direction === 'down' && currentIndex < clients.length - 1) {
      const nextClient = clients[currentIndex + 1];
      await setDoc(doc(db, 'clients', client.id), { ...client, order: nextClient.order }, { merge: true });
      await setDoc(doc(db, 'clients', nextClient.id), { ...nextClient, order: client.order }, { merge: true });
      toast.success('Order updated');
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-3xl font-bold text-brand-blue">Clients & Partners</h1>
          <p className="text-gray-500">Manage your international client companies, their reviews, and success stories.</p>
        </div>
        <button 
        ...
        {isAdding && (
          ...
          >
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-xl font-bold text-brand-blue">{editingClient ? 'Edit Client/Partner' : 'Add New Client/Partner'}</h2>
              <button 
                onClick={() => {
                  setIsAdding(false);
                  setEditingClient(null);
            ...
            <form onSubmit={handleAddClient} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Reviewer Name</label>
                    <input 
                      ...
                      onChange={(e) => setNewClient({ ...newClient, reviewerName: e.target.value })}
                      placeholder="e.g. John Smith"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Reviewer Position</label>
                    <input 
                      className="w-full px-4 py-3 rounded-xl border border-gray-100 bg-gray-50 outline-none focus:bg-white focus:border-brand-gold transition-all text-sm font-bold"
                      value={newClient.reviewerPosition}
                      onChange={(e) => setNewClient({ ...newClient, reviewerPosition: e.target.value })}
                      placeholder="e.g. HR Manager"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Review Rating (Stars)</label>
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          key={star}
                          type="button"
                          onClick={() => setNewClient({ ...newClient, rating: star })}
                          className={cn(
                        ...
                        >
                          <Star size={16} className={cn(newClient.rating && newClient.rating >= star ? "fill-current" : "")} />
                        </button>
                      ))}
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Client Review/Testimonial</label>
                    <textarea 
                      rows={3}
                      ...
                      onChange={(e) => setNewClient({ ...newClient, review: e.target.value })}
                ...
                {/* Story Note */}
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Success Story Note</label>
                  <textarea 
                    rows={9}
                    className="w-full px-4 py-3 rounded-xl border border-gray-100 bg-gray-50 outline-none focus:bg-white focus:border-brand-gold transition-all text-sm"
                    value={newClient.storyNote}
                    onChange={(e) => setNewClient({ ...newClient, storyNote: e.target.value })}
              ...
              <div className="flex justify-end gap-4 pt-6 border-t border-gray-50">
                ...
                >
                  <Save size={20} /> {editingClient ? 'Update Client Profile' : 'Save Client Profile'}
                </button>
              </div>
            </form>
          </motion.div>
          ...
          >
            <div className="p-8 border-b border-gray-50 flex justify-between items-start">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-brand-blue leading-tight">{client.companyName}</h3>
              ...
              <div className="flex gap-2">
                <div className="flex flex-col gap-1 mr-2 opacity-0 group-hover:opacity-100 transition-all">
                  <button 
                    ...
                    className="w-8 h-8 bg-gray-50 text-gray-400 rounded-lg flex items-center justify-center hover:bg-brand-teal hover:text-white disabled:opacity-20 transition-all"
                    title="Move Up"
                  >
                ...
                <button 
                  ...
                  className="w-10 h-10 bg-gray-50 text-blue-500 rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-blue-50 hover:text-blue-600 shadow-sm"
                >
                  <Edit2 size={18} />
                </button>
                <button 
                  onClick={() => handleDelete(client.id)}
                  className="w-10 h-10 bg-gray-50 text-red-400 rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-red-50 hover:text-red-600 shadow-sm"
                >
                  <Trash2 size={18} />
                </button>
              </div>
            ...
            <div className="p-8 space-y-6 flex-grow">
              {client.review && (
                <div className="bg-brand-blue/5 p-6 rounded-2xl relative">
                  ...
                  <p className="text-gray-600 text-sm italic relative z-10">"{client.review}"</p>
                  {(client.reviewerName || client.reviewerPosition) && (
                    <div className="mt-4 pt-4 border-t border-brand-blue/10">
                      <p className="text-xs font-bold text-brand-blue">{client.reviewerName}</p>
                      <p className="text-[10px] text-gray-400 uppercase tracking-widest">{client.reviewerPosition}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {client.storyNote && (
                <div>
                  ...
                  <p className="text-gray-500 text-sm leading-relaxed">{client.storyNote}</p>
                </div>
              )}
            </div>

            <div className="px-8 py-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center mt-auto">
              <span className="text-[10px] text-gray-400 font-medium">Added on {new Date(client.createdAt).toLocaleDateString()}</span>
              <div className="flex gap-1">
                {[...Array(5)].map((_, i) => (
                  <Star 
                    ...
                    className={cn(
                      "text-brand-gold",
                      i < (client.rating || 5) ? "fill-brand-gold" : "opacity-20"
                    )} 
                  />
                ))}
              </div>
      ...
      {clients.length === 0 && !isAdding && (
        <div className="bg-white p-20 rounded-[3rem] text-center border border-dashed border-gray-200">
          ...
          <h3 className="text-2xl font-bold text-brand-blue mb-2">No Clients Added Yet</h3>
          <p className="text-gray-400 max-w-md mx-auto mb-8">Showcase your global partnerships and client testimonials to build trust with new candidates.</p>
          <button 
            onClick={() => setIsAdding(true)}
        ...
        {deleteId && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="text-2xl font-bold text-brand-blue mb-2">Confirm Deletion</h3>
              <p className="text-gray-500 mb-8">Are you sure you want to delete this client? This action cannot be undone.</p>
              <div className="flex gap-4">
                <button 
                  onClick={() => setDeleteId(null)}
                  className="flex-1 px-6 py-3.5 rounded-xl font-bold text-gray-500 hover:bg-gray-50 transition-all border border-gray-100"
                >
                  Cancel
                </button>
                <button 
