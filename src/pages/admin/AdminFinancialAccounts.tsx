...
export default function AdminFinancialAccounts() {
  ...
  const handleSaveRisk = async (uid: string) => {
    try {
      ...
      const newCands = candidates.map((c) =>
        c.uid === uid ? { ...c, riskAmount: val } : c,
      );
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-300">
          Track candidate packages, collection history, risk amounts, and
          remaining balances.
        </p>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-[#121212] text-white tracking-widest text-[10px] uppercase font-black">
              <tr>
              ...
              {loading ? (
                [1, 2, 3, 4, 5].map((i) => (
                  <tr key={i} className="animate-pulse">
                    <td
                      colSpan={7}
                      className="px-6 py-6 h-20 bg-gray-50/50"
                    ></td>
                  </tr>
                ))
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <button
                              ...
                              className="font-bold text-brand-blue hover:text-brand-gold transition-colors text-left"
                            >
                              {candidate.fullName}
                            </button>
                      ...
                      <td className="px-6 py-4">
                        ...
                        <p className="text-[9px] text-gray-400 uppercase tracking-widest">
                          {candidate.paymentHistory?.length || 0} collections
                        </p>
                      </td>
                      <td className="px-6 py-4">
                        {isEditingRisk ? (
