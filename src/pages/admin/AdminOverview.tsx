...
export default function AdminOverview() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const [jobsSnap, appsSnap, messagesSnap, candidatesSnap, diarySnap] = await Promise.all([
          ...
          getDocs(collection(db, 'contactMessages')).catch(() => ({ size: 0, docs: [] })),
          getDocs(collection(db, 'candidates')).catch(() => ({ size: 0, docs: [] })),
          getDocs(collection(db, 'diary')).catch(() => ({ size: 0, docs: [] }))
        ]);

        if (appsSnap) {
          const apps = (appsSnap as any).docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as Application));
          ...
          try {
            const last7Days = [...Array(7)].map((_, i) => {
              ...
              const count = apps.filter(app => isSameDay(new Date(app.createdAt), date)).length;
              return {
  ...
  const statCards = [
    ...
    { name: 'Diary Stories', value: stats.totalDiaryPosts, icon: <Sparkles size={24} />, color: 'bg-brand-gold' },
    { name: 'Registered Candidates', value: stats.activeCandidates, icon: <TrendingUp size={24} />, color: 'bg-green-500' },
    { name: 'Contact Messages', value: stats.totalMessages, icon: <MessageCircle size={24} />, color: 'bg-orange-500' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 md:gap-6">
        {statCards.map((stat) => (
          <div key={stat.name} className="bg-white p-5 md:p-8 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 md:gap-6">
            <div className={cn("w-12 h-12 md:w-16 md:h-16 rounded-2xl flex items-center justify-center text-white shadow-lg shrink-0", stat.color)}>
              {stat.icon}
            </div>
            <div className="overflow-hidden">
              <p className="text-[10px] md:text-sm font-bold text-gray-400 uppercase tracking-wider truncate">{stat.name}</p>
              <p className="text-xl md:text-3xl font-bold text-brand-blue">{stat.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6 md:mb-8">
            <div>
              <h3 className="text-lg md:text-xl font-bold text-brand-blue">Application Trends</h3>
              <p className="text-xs md:text-sm text-gray-400">Number of applications received in the last 7 days</p>
            </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6 md:mb-8">
            <div>
              <h3 className="text-lg md:text-xl font-bold text-brand-blue">Status Distribution</h3>
              <p className="text-xs md:text-sm text-gray-400">Current application statuses</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 md:p-8 border-b border-gray-50 flex justify-between items-center">
            <h3 className="text-lg md:text-xl font-bold text-brand-blue">Recent Applications</h3>
            <Link to="/admin/applications" className="text-brand-gold font-bold text-xs md:text-sm flex items-center gap-1">
                  ...
                  {recentApplications.map((app) => (
                    <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 md:px-8 py-4 md:py-6">
                        <p className="font-bold text-brand-blue text-sm md:text-base">{app.fullName}</p>
                        <p className="text-[10px] md:text-xs text-gray-400 truncate max-w-[150px]">{app.email}</p>
                      </td>
                      <td className="px-6 md:px-8 py-4 md:py-6">
                        <p className="text-xs md:text-sm font-medium text-gray-700">{app.jobTitle}</p>
                      </td>
                      <td className="px-6 md:px-8 py-4 md:py-6">
                        <span className={cn(
          ...
          <div className="space-y-4">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <span className="font-bold text-brand-blue">Manage Candidates</span>
              </div>
