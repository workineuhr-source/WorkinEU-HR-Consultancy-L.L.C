...
export default function AdminSystemSettings() {
  ...
  const addConfig = () => {
    ...
    setNewConfig({ provider: 'gemini', label: '', apiKey: '', modelName: 'gemini-1.5-flash', isEnabled: true });
    toast.success('New API configuration node added to stack.');
  };

  const deleteConfig = (id: string) => {
    if (settings.apiConfigs.length <= 1) {
      toast.error('At least one configuration must remain in the stack for system stability.');
      return;
    }
    ...
    setSettings({
      ...settings,
      apiConfigs: updated,
      activeConfigId: settings.activeConfigId === id ? updated[0].id : settings.activeConfigId
    });
  ...
  const setActive = (id: string) => {
    ...
    toast.info(`Routing logic shifted to: ${settings.apiConfigs.find(c => c.id === id)?.label}`);
  };
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-slate-950">
        <div className="flex flex-col items-center gap-6">
          ...
          <p className="text-white font-black uppercase tracking-[0.5em] text-xs animate-pulse">Initializing Core Settings...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 space-y-10">
          ...
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-12">
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-5xl md:text-7xl font-black tracking-tight text-white uppercase italic">System Core</h1>
                  <div className="flex items-center gap-3 mt-2">
                    <span className="w-2 h-2 rounded-full bg-brand-gold animate-pulse"></span>
                    <p className="text-brand-gold font-black text-xs uppercase tracking-[0.3em]">Infrastructure Management Dashboard</p>
                  </div>
                </div>
              </div>
              <p className="text-slate-400 text-lg font-bold max-w-3xl leading-relaxed">
                Configure your multi-provider AI stack. Managed configurations allow instant failover and routing between Gemini, OpenAI, and Custom endpoints.
              </p>
            </div>

            <div className="flex flex-wrap gap-6">
              <div className="bg-slate-800/50 backdrop-blur-md p-8 rounded-[2rem] border border-white/5 shadow-2xl">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Active Node</p>
                <div className="flex items-center gap-4">
                  ...
                  <p className="text-2xl font-black text-white truncate max-w-[200px]">
                    {settings.apiConfigs.find(c => c.id === settings.activeConfigId)?.label || 'None Selected'}
                  </p>
                </div>
              </div>
              <div className="bg-slate-800/50 backdrop-blur-md p-8 rounded-[2rem] border border-white/5 shadow-2xl">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <p className="text-2xl font-black text-white">{settings.apiConfigs.length} Clusters</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-8 space-y-10">
          <div className="flex justify-between items-center bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100">
            <div>
              ...
              <p className="text-slate-500 font-bold text-xs uppercase tracking-widest mt-2">Manage and monitor all deployed API endpoints</p>
            </div>
            <button 
              ...
              {settings.apiConfigs.map((config) => (
                <motion.div 
                  ...
                  className={cn(
                    ...
                    settings.activeConfigId === config.id ? "border-brand-gold shadow-2xl ring-4 ring-brand-gold/5" : "border-slate-100 shadow-sm hover:border-slate-300"
                  )}
                >
                  ...
                  <div className="flex flex-col md:flex-row justify-between gap-10 relative z-10">
                    <div className="flex gap-8 items-start">
                      <div className={cn(
                        ...
                        config.isEnabled ? "bg-slate-950 border-emerald-500/30 text-emerald-500" : "bg-slate-100 border-slate-200 text-slate-400"
                      )}>
                        {config.provider === 'gemini' ? <Zap size={32} /> : config.provider === 'openai' ? <Cpu size={32} /> : <Globe size={32} />}
                      </div>
                      <div className="space-y-3">
                        <div className="flex items-center gap-3">
                          <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight">{config.label}</h3>
                          {!config.isEnabled && (
                            <span className="bg-slate-100 text-slate-400 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">OFFLINE</span>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-4">
                          <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <Cpu size={12} className="text-brand-gold" /> {config.provider}
                          </div>
                          <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <Settings2 size={12} className="text-brand-gold" /> {config.modelName || 'System Default'}
                          </div>
                        </div>
                        <div className="flex items-center gap-3 pt-2">
                           <div className="relative group/key">
                            <p className="text-xs font-mono text-slate-300 bg-slate-50 px-3 py-2 rounded-lg border border-slate-100 italic transition-all group-hover/key:text-slate-600">
                              {showKey === config.id ? config.apiKey : "••••••••••••••••••••••••••••"}
                            </p>
                            <button 
                            ...
                            >
                              {showKey === config.id ? <EyeOff size={14} /> : <Eye size={14} />}
                            </button>
                           </div>
                        </div>
                      </div>
                    ...
                    <div className="flex md:flex-col justify-end gap-3 min-w-[200px]">
                      <div className="flex gap-2">
                        <button 
                          ...
                          disabled={!config.isEnabled || settings.activeConfigId === config.id}
                          className={cn(
                            ...
                            settings.activeConfigId === config.id 
                              ...
                              : config.isEnabled 
                                ? "bg-white text-slate-900 border-slate-200 hover:border-brand-gold hover:text-brand-gold" 
                                : "bg-slate-50 text-slate-400 border-transparent cursor-not-allowed"
                          )}
                        >
                          {settings.activeConfigId === config.id ? 'Live Status' : 'Set Active'}
                        </button>
                        <button 
                        ...
                        >
                          {config.isEnabled ? <Power size={20} /> : <PowerOff size={20} />}
                        </button>
                      </div>
                      <button 
        ...
        <div className="lg:col-span-4 space-y-10">
          <div className="bg-slate-900 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden">
            ...
            <h4 className="text-xl font-black uppercase tracking-tight mb-8 flex items-center gap-3">
              <ShieldCheck className="text-brand-gold" size={24} /> Security Protocol
            </h4>
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex gap-4">
                  ...
                  <p className="text-xs font-bold text-slate-400 leading-relaxed uppercase">{text}</p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-12 pt-12 border-t border-white/5">
              ...
              >
                {saving ? <Loader2 className="animate-spin" size={20} /> : <><Save size={20} /> Push Configuration</>}
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[3rem] p-10 border border-slate-100 space-y-6">
             <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Stack Operations</h4>
             </div>
             <p className="text-xs text-slate-500 font-bold leading-relaxed">
                Adding multiple configurations allows for seamless transition between development and production keys or between different AI providers without code modifications.
             </p>
             <ul className="space-y-4 pt-4">
                <li className="flex items-center gap-3 text-[10px] font-black text-emerald-600 uppercase tracking-widest">
                  <CheckCircle2 size={14} /> Only enabled nodes can be set to active
                </li>
            ...
            >
              <div className="w-full md:w-[40%] bg-slate-950 p-12 text-white flex flex-col justify-between">
                <div>
                  ...
                  <h2 className="text-4xl font-black tracking-tight uppercase italic mb-6 leading-tight">Provision New Node</h2>
                  <p className="text-slate-400 font-bold text-xs uppercase tracking-widest leading-relaxed">
                    Extend the intelligence cluster by adding a new provider endpoint to the configuration stack.
                  </p>
                </div>
                <div className="pt-12">
              ...
              <div className="w-full md:w-[60%] p-12 space-y-8 max-h-[80vh] overflow-y-auto">
                <div className="grid grid-cols-1 gap-8">
                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Descriptive Label</label>
                    <input 
                      type="text" 
                      className="w-full px-8 py-5 rounded-3xl border border-slate-100 bg-slate-50 outline-none focus:bg-white focus:border-brand-gold transition-all text-sm font-bold placeholder:text-slate-300"
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Intelligence Provider</label>
                    <div className="grid grid-cols-3 gap-4">
                      {['gemini', 'openai', 'custom'].map(p => (
                        <button
                          key={p}
                          onClick={() => setNewConfig({...newConfig, provider: p as any})}
                          className={cn(
                            "py-4 rounded-2xl border-2 font-black text-[10px] uppercase tracking-widest transition-all",
                            newConfig.provider === p ? "bg-slate-950 text-white border-slate-950" : "bg-slate-50 border-transparent text-slate-400"
                          )}
                        >
                          {p}
                        </button>
                  ...
                  <div className="space-y-4">
                    ...
                    <input 
                      type="password" 
                      className="w-full px-8 py-5 rounded-3xl border border-slate-100 bg-slate-50 outline-none focus:bg-white focus:border-brand-gold transition-all text-sm font-bold"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    {newConfig.provider === 'custom' && (
                      <div className="space-y-4">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Base API Endpoint</label>
                        <input 
                          type="text" 
                          className="w-full px-8 py-5 rounded-3xl border border-slate-100 bg-slate-50 outline-none focus:bg-white focus:border-brand-gold transition-all text-sm font-bold"
