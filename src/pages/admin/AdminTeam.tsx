...
export default function AdminTeam() {
  ...
  const fetchTeam = async () => {
    try {
      ...
      const teamData = querySnapshot.docs.map(
  ...
  const handleMove = async (index: number, direction: "up" | "down") => {
    ...
    try {
      await Promise.all([
        updateDoc(doc(db, "team", newTeam[index].id), {
          order: newTeam[index].order,
        }),
        updateDoc(doc(db, "team", newTeam[targetIndex].id), {
          order: newTeam[targetIndex].order,
        }),
      ]);
      fetchTeam();
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-300 text-sm md:text-base">
            Manage your team members, their positions, and contact info.
          </p>
        </div>
        ...
        {team.map((member, index) => (
          ...
          >
            ...
            <div className="aspect-square relative">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                <button
                  ...
                  className="p-3 bg-white dark:bg-[#121212] text-brand-blue rounded-full hover:bg-brand-gold transition-colors shadow-lg"
                >
                  <Edit2 size={20} />
                </button>
                <button
                  onClick={() => setDeleteId(member.id)}
                  className="p-3 bg-white dark:bg-[#121212] text-red-500 rounded-full hover:bg-red-50 transition-colors shadow-lg"
                >
                  <Trash2 size={20} />
                </button>
              </div>
            ...
            <div className="p-5 md:p-6">
              <h3 className="text-lg md:text-xl font-bold text-brand-blue mb-1">
                {member.name}
              </h3>
              <p className="text-brand-gold font-medium mb-3 md:mb-4 text-sm md:text-base">
                {member.position}
              </p>
              <div className="flex items-center gap-2 text-gray-500 dark:text-gray-300 text-xs md:text-sm">
                <Phone size={14} className="md:w-4 md:h-4" />
                <span>{member.whatsapp}</span>
              </div>
              {member.email && (
                <div className="flex items-center gap-2 text-gray-500 dark:text-gray-300 text-xs md:text-sm mt-1">
          ...
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
            ...
            >
              <div className="flex flex-col items-center gap-6">
                <label className="block text-xs md:text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-widest">
                  Team Member Identity
                </label>
                <div className="flex flex-col sm:flex-row items-center gap-6 w-full">
                  <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-gray-100 bg-gray-50 dark:bg-white/5 group shrink-0">
                    {formData.photoUrl ? (
                      <img
                        referrerPolicy="no-referrer"
                  ...
                  <div className="flex-grow w-full space-y-2">
                    ...
                    <p className="text-[9px] text-gray-400 italic px-1">
                      Tip: You can use direct links from Facebook, LinkedIn, or
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs md:text-sm font-bold text-gray-700 dark:text-gray-200 mb-1">
                    Full Name
                  </label>
                  <input
                    required
